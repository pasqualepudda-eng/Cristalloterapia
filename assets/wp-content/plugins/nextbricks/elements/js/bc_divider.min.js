...
if(animationType=='Click'&&parseFloat(classTriggerClick)!=0){document.querySelectorAll(classTriggerClick).forEach(function(e){e.addEventListener("click",function(){t[`g${r}`].restart()})})}else if(animationType=='Hover'&&parseFloat(classTriggerHover)!=0){document.querySelectorAll(classTriggerHover).forEach(function(e){e.addEventListener("mouseover",function(){t[`g${r}`].restart()})})}})}
...
document.addEventListener("DOMContentLoaded",function(t){document.fonts.ready.then(function(){bricksIsFrontend&&bc_divider()})})